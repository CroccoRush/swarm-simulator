...
def combine_data(data_list):
    ...
    grouped_data = (
        combined_data.groupby("time_rounded")[columns_to_average]
        .mean()
        .reset_index()
    )
    ...
    time_avg = (
        combined_data.groupby("time_rounded")["time"].mean().reset_index()
    )
    ...
    for id in range(num_drones):
        ...
        one_data[["x", "y"]] = one_data.apply(
            lambda row: pd.Series(
                latlon_to_meters(
                    row["lat"] * 1e-7, row["lon"] * 1e-7, lat0, lon0
                )
            ),
            axis=1,
        )
...
def plot_trajectories(time_to_data1, time_to_data2, num_drones):
    ...
    ax.set_title(
        "Drone Trajectories Comparison (Meters from Reference)", fontsize=14
    )
...
def calculate_formation_error(drones_positions):
    ...
    for src_drone in drones_positions:
        ...
        for i, d in zip(
            indx[1:count], dist[1:count]  # skip first because its src drone
        ):
            ...
            target_delta = [
                target_drone[0] - src_drone[0],
                target_drone[1] - src_drone[1],
            ]
            min_dist = min(
                [
                    distance(target_pos, target_delta)
        ...
        for t, e in zip(times, errors):
            if t >= start_time:
                filtered_times.append(
                    (t - start_time) / 10**9
                )  # Time in seconds from the start of the active phase
...
def plot_specific_experiments(experiment_nums):
    """
    Plots formation errors for specific experiments comparing LVP and ALVP
    algorithms.
    ...
    if all_times:
        ...
        # Add vertical lines
        time_markers = [
            max(all_times) - seconds for seconds in [66.5, 65, 45, 30, 10]
        ]
        add_vertical_time_markers(plt.gca(), time_markers, y_max)
    ...
    plt.title(
        "Formation Error Comparison for Specific Experiments: LVP vs ALVP"
    )
    ...
    print(
        "Saved specific experiments comparison graph to "
        "specific_experiments_comparison.png"
    )
    plt.close()
...
def plot_single_experiment(exp_num: int):
    """
    Plots formation errors for single experiment comparing LVP and ALVP
    algorithms.
    ...
    print(
        "Saved single experiments comparison graph to "
        "single_experiments_comparison.png"
    )
    plt.close()
...
def get_rotated_coordinates(drone_data, avg_heading):
    """
    Gets the rotated coordinates of the drones to align them according to the
    average heading.
...
def average(n_experiments=10):
    """
    Builds a graph comparing average formation errors for LVP and ALVP
    algorithms.
        ...
        for exp_num in range(1, n_experiments + 1):
            ...
            try:
                time_to_data = get_data(
                    DRONE_COUNT, f"_set{param_set}_exp{exp_num}"
                )
                ...
                print(
                    f"Error processing experiment set{param_set}_exp{exp_num}: "
                    f"{e}"
                )
                continue
    ...
    all_times = sorted(
        set(list(lvp_avg_errors.keys()) + list(alvp_avg_errors.keys()))
    )
    if not all_times:
    ...
    if lvp_times and lvp_errors:
        plt.plot(
            lvp_times, lvp_errors, "b-", linewidth=2, label="LVP Algorithm"
        )

    if alvp_times and alvp_errors:
        plt.plot(
            alvp_times, alvp_errors, "r-", linewidth=2, label="ALVP Algorithm"
        )
    ...
    if all_times_for_plot:
        ...
        # Add vertical lines
        time_markers = [
            max(all_times_for_plot) - seconds
            for seconds in [66.5, 65, 45, 30, 10]
        ]
        add_vertical_time_markers(plt.gca(), time_markers, y_max)
    ...
    print(
        "Saved average error comparison graph to average_error_comparison.png"
    )
    plt.close()
    ...
    if lvp_errors:
        print(
            f"LVP - Mean: {np.mean(lvp_errors):.2f}m, Std: "
            f"{np.std(lvp_errors):.2f}m"
        )
    ...
    if alvp_errors:
        print(
            f"ALVP - Mean: {np.mean(alvp_errors):.2f}m, Std: "
            f"{np.std(alvp_errors):.2f}m"
        )
    ...
    if lvp_errors and alvp_errors:
        ...
        print(
            f"Difference (LVP - ALVP): {lvp_integral - alvp_integral:.2f} m·s"
        )
...
def create_boxplot(data, labels, title, output_filename, time_markers=None):
    ...
    # Configure the labels on the X axis - show every 20 labels for readability
    if len(labels) > 20:
        step = len(labels) // 20
        plt.xticks(
...
def plot_error_lines(error_data_by_exp, algorithm):
    ...
    if times_sec:
        time_markers = [
            max(times_sec) - seconds for seconds in [66.5, 65, 45, 30, 10]
        ]
...
def process_error_data_by_time(errors_by_time, start_time):
    ...
    # Filter data by time
    filtered_data = {
        t: errors for t, errors in errors_by_time.items() if t >= start_time
    }
        ...
        for i, t in enumerate(sorted_times):
            if t in filtered_data:
                ...
                if original_median > 0:
                    ...
                    filtered_data[t] = [
                        e * correction_factor for e in filtered_data[t]
                    ]
...
def analyze_logs(n_experiments=10):
        ...
        for exp_num in range(1, n_experiments + 1):
            ...
            try:
                ...
                for time, drone_data in time_to_data.items():
                    ...
                    rotated_coords = get_rotated_coordinates(
                        drone_data, avg_heading
                    )
                    formation_error = calculate_formation_error(rotated_coords)

                    if param_set == 1:
                        lvp_errors_by_time[time_rounded].append(
                            formation_error
                        )
                    else:
                        alvp_errors_by_time[time_rounded].append(
                            formation_error
                        )
                ...
                if (
                    len(exp_errors) > 5
                ):  # Apply smoothing only if there is enough data
    ...
    # Determine the start time of the active phase
    # (after half of the experiment time)
    all_times = []
    ...
    _, lvp_data, lvp_labels = process_error_data_by_time(
        lvp_errors_by_time, start_time
    )
    _, alvp_data, alvp_labels = process_error_data_by_time(
        alvp_errors_by_time, start_time
    )
    ...
    # Build graphs of formation error lines for individual experiments
    # (starting from half of the time)
    ...
    if lvp_data:
        ...
        print(
            f"LVP - Mean: {np.mean(flat_lvp):.2f}m, Std: "
            f"{np.std(flat_lvp):.2f}m"
        )
    if alvp_data:
        flat_alvp = [item for sublist in alvp_data for item in sublist]
        print(
            f"ALVP - Mean: {np.mean(flat_alvp):.2f}m, Std: "
            f"{np.std(flat_alvp):.2f}m"
        )
...
def drone_paths(target_exp):
    ...
    for param_set in [1, 2]:  # 1 = LVP, 2 = ALVP
        try:
            ...
            drone_trajectories = {
                i: {"times": [], "x": [], "y": []} for i in range(DRONE_COUNT)
            }
            ...
            print(
                f"Error processing experiment set{param_set}_exp{target_exp}: "
                f"{e}"
            )
            continue
    ...
    if lvp_trajectories:
        for drone_id, data in lvp_trajectories.items():
            if data["x"] and data["y"]:
                ...
                ax1.scatter(
                    data["x"][0], data["y"][0], color=color, s=100, marker="x"
                )
    else:
    ...
    fig.legend(
        handles, labels, loc="upper center", ncol=5, bbox_to_anchor=(0.5, 0.05)
    )
...
""",
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["boxplot", "average", "paths", "specific", "single"],
        default="boxplot",
        help="Analysis mode: "
        "boxplot (Box plots and per-experiment error graphs), "
    ...
    if args.mode == "boxplot":
        ...
        print(
            f"Running average error graph mode with {args.num_experiments} "
            "experiments..."
        )
        ...
        print(
            "Running specific experiments mode with experiments: "
            f"{args.experiments}..."
        )
        plot_specific_experiments(args.experiments)
    elif args.mode == "single":
        print(
            "Running single experiment mode with experiment: "
            f"{args.target_exp}..."
        )
